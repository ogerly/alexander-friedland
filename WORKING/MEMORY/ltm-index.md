...
| 2026-09-23 | WP-2026-09-23-GOV-PUSH-Token-Block-Security-Cleanup.md | GOV | **Token-Vorfall:** `.env`-Token von GitHub abgelehnt (401 „Bad credentials"), war ~30 min vorher noch gültig (Pushes 07:40 + 13:00 OK) → Expiry erreicht oder GitHub-Auto-Revoke vermutet; neuer PAT (Scope `repo`) ausstehend. Push `865ac69` (Märchen-Artikel) blockiert, lokal committed. **User-Regel (bindend, D1): Token nur in `.env`** — AAMS-Clone origin-Remote bereinigt (alter PAT entfernt, `.git/config` sauber), Token-Präfix-Referenz in Workpaper entfernt; False-Positives: `.agent.json` = mask_patterns, WP-2026-09-16 = maskierter Verweis. AAMS-Repo-Diary-Zeile liegt lokal (Session-Close dort = 09-22-Sitzung). |
